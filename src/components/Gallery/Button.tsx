...
export default function Button({ children, onClick, url, target, className, ...rest }: Props) {
	return (
		<a
			...
			onClick={onClick}
			className={`w-fit rounded-full border border-solid border-white px-3 py-3 text-xs font-medium uppercase no-underline md:px-5 md:text-xl lg:text-2xl ${styles.button} ${className ?? ""}`}
			{...rest}
		>
			{children}
		</a>
